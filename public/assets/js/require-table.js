        ...
        api: {
            ...
            events: {
                operate: {
                    ...
                    'click .btn-show': function (e, value, row, index) {
                        ...
                        Fast.api.open(Table.api.replaceurl(url, row, table), __('maintenance log')+'(device_no：'+row.device_no+')', $(this).data() || {});
                    },
                    ...
                    'click .btn-bind': function (e, value, row, index) {
                        ...
                        var ids = row[options.pk];
                        row = $.extend({}, row ? row : {}, {ids: ids});
                        var url = options.extend.bind_url;
                        Fast.api.open(Table.api.replaceurl(url, row, table), __('关联管理')+'(id：'+row.id+')', $(this).data() || {});
                    },

                    //打开扣分细目
                    'click .btn-rank-list': function (e, value, row, index) {
                        e.stopPropagation();
                        e.preventDefault();
                        var table = $(this).closest('table');
                        var options = table.bootstrapTable('getOptions');
                        var ids = row[options.pk];
                        row = $.extend({}, row ? row : {}, {ids: ids});
                        var url = options.extend.rank_list_url;
                        Fast.api.open(Table.api.replaceurl(url, row, table), __('扣分细目')+'[id：'+row.id+']'+row.year+''+'-'+row.quarter+''+'-'+row.institution+'', $(this).data() || {});
                    },
            ...
            formatter: {
                ...
                operate: function (value, row, index) {
                    ...
                    //隐藏操作按钮的特殊配置
                    if ( options.extend.index_url.indexOf('rank_breakdown') !== -1 && row.name == '逾期' || row.name == '作业不规范' ){
                        return '此项无法更改';
                    }else{
