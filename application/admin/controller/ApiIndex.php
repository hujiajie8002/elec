...
class ApiIndex extends Controller
{
    ...
    //获取结果数据，修改版，由之前通过思创接口获取改为从本地数据库获取
    public  function getReport(){
        ...
        $data = Db::table('dky_report_data')->where('s_code',$test_code)->order('update_at desc')->find();
        if(isset($data['type'])){
            $res['type'] = $data['type'];
        }

        if(isset($data['json'])){
            $json_arr = json_decode($data['json'],true);
            $res['data'] = $json_arr['testDataMapPlus'];
        }
...
<string xmlns="http://tempuri.org/">{"样品信息":[{"试品编号":"byq0001","样品名称":"油浸式变压器","样品描述":"","批次":"","型号规格":"","任务编号":"","报告编号":"","盲样编号":"","委托单位":"","样品编号":"byq0001","生产厂商":"","试验人员":"","检测日期":"2020-08-29T20:14:09"}],"参数信息":[{"高压侧低分接电压":"10.50","高压侧额定电压":"10","高压侧高分接电压":"9.50","变压器联结方式":"D/yn","环温探头":"4","高压侧额定电流":"23.1","低压侧额定电压":"400","低压侧额定电流":"577.4","阻抗电压":"3.98","额定容量":"400","额定频率":"50","额定变比":"25","分接间距":"2.5","分接范围":"±2×2.5","组别号":"11","分接总数":"5","测试分接":"3","相数":"三相","冷却方式":"ONAN(油浸式)","高压侧试验电流":"自动","低压侧试验电流":"自动","负载测量分接":"3","最高温升":"75","油温探头":"1","高压侧顶部探头":"5","高压侧底部探头":"6","绕组探头":"9;10","铁芯探头":"7","套管探头":"8","低对高及地":"5","高对低及地":"10","持续时间":"60","变压器类型":"非晶","绝缘等级":"A","噪音水平":"60","托盘高度":"200","变压器高度":"832","高压测套管高度":"245","低压测套管高度":"126","高压测螺柱高度":"30","低压测螺柱高度":"43"}],"检验标准":[],"检验标准2":[],"检测仪器":[],"绕组绝缘电阻试验":[],"绕组绝缘电阻试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"外施耐压试验":[],"外施耐压试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"感应耐压试验":[],"感应耐压试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"绕组直流电阻试验":[],"绕组直流电阻试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"电压比测量及联结组标号检定试验":[],"电压比测量及联结组标号检定试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"空载电流及空载损耗试验":[],"空载电流及空载损耗试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"在90%和110额定电压下的空载损耗和空载电流测量":[],"在90%和110额定电压下的空载损耗和空载电流测量结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"声级试验":[],"声级试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"短路阻抗及负载损耗试验":[],"短路阻抗及负载损耗试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"油变温升试验":[],"油变温升试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"短时过负载试验":[],"短时过负载试验结果":[{"任务号":null,"试验状态":"","试验日期":null,"油温":null,"环境温度":null,"相对湿度":null}],"生成报告":[{"URL":"http://192.168.18.3/rxjc_web/page/report.aspx?testcode=byq0001&amp;mrtfile=油浸式变压器&amp;request=0"}]}</string>';
    ...
    public function saveProcessData()
    {
        ...
        //处理并存储关键过程数据，仅温升试验的关键过程数据需要存储
        if ($experiment_name == '温升试验'){
            $this->disposeMainProcessData($array);
        }
    ...
    //处理并存储关键过程数据
    public function disposeMainProcessData($array)
    {
        $type = $array['type'];
        $sample_barcode = $array['sample_barcode'];
        $experiment_name = $array['experiment_name'];
        $json = $array['json'];
        $json_arr = json_decode($json,true)[0];
        //筛选好的关键过程数据数组
        $key_json = [];
        ...
        //获取上一次存储到现在的间隔时间
        $newest_record = Db::table('main_process_data')->where('sample_barcode',$sample_barcode)->order('id desc')->find();
        //最新一条记录的插入时间
        ...
        switch ($type){
            case '柱上开关设备':
                $key_json['时间'] = isset($json_arr['时间']) ? $json_arr['时间'] : '';
                $key_json['施加电流'] = isset($json_arr['施加电流']) ? $json_arr['施加电流'] . 'A' : '';
                $key_json['进线端子A相温度'] = isset($json_arr['进线端子A相温度']) ? $json_arr['进线端子A相温度'].'℃' : '';
                $key_json['进线端子B相温度'] = isset($json_arr['进线端子B相温度']) ? $json_arr['进线端子B相温度'].'℃' : '';
                $key_json['进线端子C相温度'] = isset($json_arr['进线端子C相温度']) ? $json_arr['进线端子C相温度'].'℃' : '';
                $key_json['出线端子A相温度'] = isset($json_arr['出线端子A相温度']) ? $json_arr['出线端子A相温度'].'℃' : '';
                $key_json['出线端子B相温度'] = isset($json_arr['出线端子B相温度']) ? $json_arr['出线端子B相温度'].'℃' : '';
                $key_json['出线端子C相温度'] = isset($json_arr['出线端子C相温度']) ? $json_arr['出线端子C相温度'].'℃' : '';
                $key_json['距进线端子1m处连接导体A相温度'] = isset($json_arr['距进线端子1m处连接导体A相温度']) ? $json_arr['距进线端子1m处连接导体A相温度'].'℃' : '';
                $key_json['距进线端子1m处连接导体B相温度'] = isset($json_arr['距进线端子1m处连接导体B相温度']) ? $json_arr['距进线端子1m处连接导体B相温度'].'℃' : '';
                $key_json['距进线端子1m处连接导体C相温度'] = isset($json_arr['距进线端子1m处连接导体C相温度']) ? $json_arr['距进线端子1m处连接导体C相温度'].'℃' : '';
                $key_json['距出线端子1m处连接导体A相温度'] = isset($json_arr['距出线端子1m处连接导体A相温度']) ? $json_arr['距出线端子1m处连接导体A相温度'].'℃' : '';
                $key_json['距出线端子1m处连接导体B相温度'] = isset($json_arr['距出线端子1m处连接导体B相温度']) ? $json_arr['距出线端子1m处连接导体B相温度'].'℃' : '';
                $key_json['距出线端子1m处连接导体C相温度'] = isset($json_arr['距出线端子1m处连接导体C相温度']) ? $json_arr['距出线端子1m处连接导体C相温度'].'℃' : '';
                $key_json['可触及外壳1温度'] = isset($json_arr['可触及外壳1温度']) ? $json_arr['可触及外壳1温度'].'℃' : '';
                $key_json['可触及外壳2温度'] = isset($json_arr['可触及外壳2温度']) ? $json_arr['可触及外壳2温度'].'℃' : '';
                $key_json['环境温度1'] = isset($json_arr['环境温度1']) ? $json_arr['环境温度1'].'℃' : '';
                $key_json['环境温度2'] = isset($json_arr['环境温度2']) ? $json_arr['环境温度2'].'℃' : '';
                $key_json['环境温度3'] = isset($json_arr['环境温度3']) ? $json_arr['环境温度3'].'℃' : '';
                $key_json['环境温度4'] = isset($json_arr['环境温度4']) ? $json_arr['环境温度4'].'℃' : '';

                if ($interval >= 1800) {
                    $array['json'] = json_encode($key_json,320);
                    $res = Db::table('main_process_data')->insert($array);
                }
                break;
            case '高压电缆分支箱':
                $key_json['时间'] = isset($json_arr['时间']) ? $json_arr['时间'] : '';
            ...
            case '环网柜':
                ...
                $key_json['环境温度3'] = isset($json_arr['环境温度3']) ? $json_arr['环境温度3'].'℃' : '';
                $key_json['环境温度4'] = isset($json_arr['环境温度4']) ? $json_arr['环境温度4'].'℃' : '';
                $key_json['气箱温度'] = isset($json_arr['气箱温度']) ? $json_arr['气箱温度'].'℃' : '';
            ...
            case '高压开关柜':
                ...
                $key_json['距出线端子1m处连接导体B相温度'] = isset($json_arr['距出线端子1m处连接导体B相温度']) ? $json_arr['距出线端子1m处连接导体B相温度'].'℃' : '';
                $key_json['距出线端子1m处连接导体C相温度'] = isset($json_arr['距出线端子1m处连接导体C相温度']) ? $json_arr['距出线端子1m处连接导体C相温度'].'℃' : '';
                $key_json['外壳温度1温度'] = isset($json_arr['外壳温度1温度']) ? $json_arr['外壳温度1温度'].'℃' : '';
                $key_json['外壳温度2温度'] = isset($json_arr['外壳温度2温度']) ? $json_arr['外壳温度2温度'].'℃' : '';
                $key_json['环境温度1'] = isset($json_arr['环境温度1']) ? $json_arr['环境温度1'].'℃' : '';
                $key_json['环境温度2'] = isset($json_arr['环境温度2']) ? $json_arr['环境温度2'].'℃' : '';
                $key_json['环境温度3'] = isset($json_arr['环境温度3']) ? $json_arr['环境温度3'].'℃' : '';
                $key_json['环境温度4'] = isset($json_arr['环境温度4']) ? $json_arr['环境温度4'].'℃' : '';
                $key_json['手车触头（上）A相温度'] = isset($json_arr['手车触头（上）A相温度']) ? $json_arr['手车触头（上）A相温度'].'℃' : '';
                $key_json['手车触头（下）A相温度'] = isset($json_arr['手车触头（下）A相温度']) ? $json_arr['手车触头（下）A相温度'].'℃' : '';
                $key_json['手车触头（上）B相温度'] = isset($json_arr['手车触头（上）B相温度']) ? $json_arr['手车触头（上）B相温度'].'℃' : '';
                $key_json['手车触头（上）C相温度'] = isset($json_arr['手车触头（上）C相温度']) ? $json_arr['手车触头（上）C相温度'].'℃' : '';
                $key_json['手车触头（下）B相温度'] = isset($json_arr['手车触头（下）B相温度']) ? $json_arr['手车触头（下）B相温度'].'℃' : '';
                $key_json['手车触头（下）C相温度'] = isset($json_arr['手车触头（下）C相温度']) ? $json_arr['手车触头（下）C相温度'].'℃' : '';
            ...
            case '配电变压器':
                //分阶段
                $period = isset($json_arr['阶段名称']) ? $json_arr['阶段名称'] : '';
                //总损耗阶段30分字存一次，额定电流阶段5分钟存一次
                if ($period){
                    if ($period == '总损耗'){
                        $key_json['施加电压'] = isset($json_arr['施加电压']) ? $json_arr['施加电压'].'kV' : '';
                        $key_json['施加电流'] = isset($json_arr['施加电流']) ? $json_arr['施加电流'].'A' : '';
                        $key_json['施加功率'] = isset($json_arr['施加功率']) ? $json_arr['施加功率'].'kW' : '';
                        $key_json['顶层液体温度'] = isset($json_arr['顶层液体温度']) ? $json_arr['顶层液体温度'].'℃' : '';
                        $key_json['顶部液体温度1'] = isset($json_arr['顶部液体温度1']) ? $json_arr['顶部液体温度1'].'℃' : '';
                        $key_json['顶部液体温度2'] = isset($json_arr['顶部液体温度2']) ? $json_arr['顶部液体温度2'].'℃' : '';
                        $key_json['顶部液体温度平均值'] = isset($json_arr['顶部液体温度平均值']) ? $json_arr['顶部液体温度平均值'].'℃' : '';
                        $key_json['底部液体温度1'] = isset($json_arr['底部液体温度1']) ? $json_arr['底部液体温度1'].'℃' : '';
                        $key_json['底部液体温度2'] = isset($json_arr['底部液体温度2']) ? $json_arr['底部液体温度2'].'℃' : '';
                        $key_json['底部液体温度平均值'] = isset($json_arr['底部液体温度平均值']) ? $json_arr['底部液体温度平均值'].'℃' : '';
                        $key_json['环境温度1'] = isset($json_arr['环境温度1']) ? $json_arr['环境温度1'].'℃' : '';
                        $key_json['环境温度2'] = isset($json_arr['环境温度2']) ? $json_arr['环境温度2'].'℃' : '';
                        $key_json['环境温度3'] = isset($json_arr['环境温度3']) ? $json_arr['环境温度3'].'℃' : '';
                        $key_json['环境温度4'] = isset($json_arr['环境温度4']) ? $json_arr['环境温度4'].'℃' : '';
                        //获取上一次存储到现在的间隔时间
                        $newest_record = Db::table('main_process_data')
                            ->where('sample_barcode',$sample_barcode)
                            ->where('period','总损耗')
                            ->order('id desc')
                            ->find();
                        //最新一条记录的插入时间
    ...
    public function getStationStatus()
    {
        ...
        if (strpos($res,'<') !== false)
        {
            ...
            foreach ($station_array as $k=>$v)
            {
                ...
                if ($v['static'] == '未开启'){
                    $arr['gz'][] = ['name'=>$station_name,'principal'=>$principal,'status'=>'未开启'];
                    $arr['sum'][] = ['name'=>$station_name,'principal'=>$principal,'status'=>'未开启'];
